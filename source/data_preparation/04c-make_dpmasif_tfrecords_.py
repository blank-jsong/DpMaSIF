...
with tf.python_io.TFRecordWriter(
    os.path.join(tfrecords_dir, "training_data_sequenceSplit_30_dpmasif.tfrecord")
) as writer:
    for i, pdb in enumerate(train_pdbs):
        ...
        try:
            ...
            mapped_point = np.load(os.path.join(
                precom_dir,pdb+"_","mapped_point.npy"
                )
            )
        except:
            continue
        ...
        mapped_point_shape = tf.train.Int64List(value=mapped_point.shape)
        mapped_point = tf.train.Int64List(value=mapped_point)
        ...
        features_dict = {
            ...
            "mapped_point_shape":tf.train.Feature(int64_list=mapped_point_shape),
            "mapped_point":tf.train.Feature(int64_list=mapped_point),
        }
...
success = 0
with tf.python_io.TFRecordWriter(
    os.path.join(tfrecords_dir, "validation_data_sequenceSplit_30_dpmasif.tfrecord")
) as writer:
...
success = 0
with tf.python_io.TFRecordWriter(
    os.path.join(tfrecords_dir, "testing_data_sequenceSplit_30_dpmasif.tfrecord")
) as writer:
