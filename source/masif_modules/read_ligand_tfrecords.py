...
def _parse_function(example_proto):
    keys_to_features = {
        ...
        "mapped_point_shape":tf.FixedLenFeature([1], dtype=tf.int64),
        "mapped_point":tf.VarLenFeature(dtype=tf.int64),
    }
    ...
    mapped_point = tf.sparse_tensor_to_dense(parsed_features["mapped_point"])
    mapped_point = tf.reshape(
        mapped_point, tf.cast(parsed_features["mapped_point_shape"], tf.int32)
    )
    return (
        input_feat,
        rho_wrt_center,
