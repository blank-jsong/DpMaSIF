...
from masif_modules.DpMaSIF import DpMaSIF
...
training_data = tf.data.TFRecordDataset(
    os.path.join(params["tfrecords_dir"], "training_data_sequenceSplit_30_dpmasif.tfrecord")
)
validation_data = tf.data.TFRecordDataset(
    os.path.join(params["tfrecords_dir"], "validation_data_sequenceSplit_30_dpmasif.tfrecord")
)
testing_data = tf.data.TFRecordDataset(
    os.path.join(params["tfrecords_dir"], "testing_data_sequenceSplit_30_dpmasif.tfrecord")
)
...
output_model = out_dir + "model_dpmasif"
if not os.path.exists(params["model_dir"]):
    os.makedirs(params["model_dir"])

with tf.Session() as sess:
    # Build the neural network model
    learning_obj = DpMaSIF(
        sess,
        params["max_distance"],
        params["n_classes"],
        idx_gpu="/gpu:0",
    ...
    if os.path.exists(output_model + ".meta"):
        learning_obj.saver.restore(sess, output_model)
    ...
    total_iterations = 0
    num_epochs = 100
    for num_epoch in tqdm(range(num_epochs),desc='Training'):
        # 记录开始时间
        ...
        for num_train_sample in range(int(num_training_samples / 10)):
            ...
            ligand_types = np.unique(labels)
            ligand_types = ligand_types[ligand_types != 0]
            n_ligands = labels.shape[1]
            labels_index = ligand_types - 1
            pocket_points = data_element[5]
            ...
            training_loss, training_logits = learning_obj.session.run(
                [learning_obj.data_loss, learning_obj.logits_sigmoid],
                feed_dict=feed_dict,
            )
        ...
        for num_sample in range(num_training_samples):
            ...
            _, training_loss, norm_grad, logits, logits_sigmoid, computed_loss = learning_obj.session.run(
                [
                    learning_obj.optimizer,
                    learning_obj.data_loss,
                    learning_obj.norm_grad,
                    learning_obj.logits,
                    learning_obj.logits_sigmoid,
            ...
            print(
                "Num sample {}\tTraining loss {}\nLabels {}\tSigmoid logits {}\tComputed loss {}\n".format(
                    num_sample,
                    training_loss,
                    pocket_labels,
                    logits_sigmoid,
