...
output_model = model_dir + "model_dpmasif"
    ...
    for num_test_sample in range(num_test_samples):
        ...
        for ligand in range(n_ligands):
            ...
            for i in range(100):
                ...
                # logits_sigmoid, data_loss = learning_obj.session.run(
                #     [learning_obj.logits_sigmoid, learning_obj.data_loss],
                #     feed_dict=feed_dict,
                # )
                # samples_logits_sigmoid.append(logits_sigmoid)
            ...
            # pdb_logits_sigmoid.append(samples_logits_sigmoid)
            pdb_logits_softmax.append(samples_logits_softmax)
        # np.save(test_set_out_dir + "{}_labels_deeppocket.npy".format(pdb), pocket_labels)
        # np.save(test_set_out_dir + "{}_logits_deeppocket.npy".format(pdb), pdb_logits_sigmoid)
        np.save(test_set_out_dir + "{}_labels_masif.npy".format(pdb), pocket_labels)
        np.save(test_set_out_dir + "{}_logits_masif.npy".format(pdb), pdb_logits_softmax)
